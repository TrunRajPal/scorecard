...
func assignRawData(probeCheckName string, request *checker.CheckRequest, ret *Result) error {
	...
	case checks.CheckSecretHygiene:
		rawData, err := raw.SecretHygiene(request)
		if err != nil {
			return sce.WithMessage(sce.ErrScorecardInternal, err.Error())
		}
		ret.RawResults.SecretHygieneResults = rawData
	case checks.CheckDangerousAgentConfig:
		rawData, err := raw.AgentConfig(request)
		if err != nil {
			return sce.WithMessage(sce.ErrScorecardInternal, err.Error())
		}
		ret.RawResults.AgentConfigResults = rawData
