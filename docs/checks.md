...
## Dangerous-Agent-Config 

Risk: `High` (arbitrary local code execution; instruction injection invisible to review)

AI coding agents read configuration files from the repository and act on them:
instruction files such as `AGENTS.md`, `CLAUDE.md`, `.cursorrules` and
`.cursor/rules/*.mdc`, and Model Context Protocol server definitions such as
`.mcp.json`. These files are an instruction channel with the reach of code, but they
are reviewed as documentation, and no existing check covers them.

Two classes of construct are reported.

**Hidden instructions.** Characters that cannot legitimately appear in prose: the
Unicode Tags block (U+E0000–U+E007F), whose only current use is to smuggle text past a
human reviewer while remaining fully legible to a model, and explicit bidirectional
overrides — the Trojan Source technique of CVE-2021-42574 — where rendered text differs
from what is parsed.

**Unpinned or shell-based execution.** MCP server definitions that fetch and run a
package at launch without an exact version pin, or that invoke a shell directly.
CVE-2025-54136 ("MCPoison") demonstrated the consequence: an attacker modified a
previously approved server definition and the editor re-executed it as trusted, with no
further prompt. Unlike CI, these commands run on the developer's own machine.

The scope is deliberately narrow. This check does **not** judge whether natural-language
instructions are malicious in intent. That question has no objective oracle and
therefore no measurable false-positive rate; reporting only constructs with a definite
answer is what makes the result auditable.

Zero-width joiners and non-joiners are excluded from the hidden-character test. They are
required orthographic characters in Indic, Persian and Khmer scripts, and measurement
against real repositories showed them throughout translated documentation — flagging
them would report correct translations as attacks.

A repository with no agent configuration files is reported as inconclusive rather than
passing: crediting a project for not using AI tooling would make the check reward
absence rather than safety.
 

**Remediation steps**
- Remove invisible and bidirectional-override characters from agent instruction files. Legitimate documentation does not require them.
- Pin MCP server packages to an exact version rather than a mutable tag such as `@latest`, so that the code executed is the code that was reviewed.
- Treat agent instruction and MCP configuration files as executable content in code review: require the same scrutiny for a change to `.cursorrules` or `.mcp.json` as for a change to a build script.
...
The check separately inspects the *resolved* dependency graph via lockfiles
(`package-lock.json`, `poetry.lock`, `Pipfile.lock`). This is a different failure
mode, not a deeper version of the same one: a lockfile entry is resolved
automatically by the package manager from an already-published parent package's
own metadata, so no human or AI typed that name into the project. A lockfile
entry that fails to resolve indicates a lockfile-integrity problem (tampering, a
hand-edited or corrupted lockfile, or an AI tool editing the lockfile directly),
not hallucination in the sense above, and is scored separately at half the
severity of a direct-manifest hallucination to reflect that lower certainty.
...
- If the finding is a resolved-graph anomaly (a lockfile entry, not a direct manifest entry), regenerate the lockfile from a trusted manifest and verify it was not hand-edited or tampered with.
...
Risk: `Medium` (accumulated unpatched risk ahead of any assigned CVE)

This check measures how far behind the current release each exactly-pinned direct
dependency is, in both elapsed days and number of intervening releases.

No existing Scorecard check covers this. `Vulnerabilities` only fires once a CVE has
been assigned, so a dependency three years out of date with no CVE yet is invisible to
it. `Pinned-Dependencies` concerns whether a dependency is pinned by hash rather than
how old the pinned version is — a hash-pinned 2019 release scores perfectly.
`Dependency-Update-Tool` reports whether an update bot is configured, which is a process
signal rather than a statement about the dependencies themselves. Staleness is a
*leading* indicator of risk; known-vulnerability is a *lagging* one.

The AI relevance is evidenced, but narrower than it may appear. Singla et al.
(arXiv:2601.00205), studying 117,062 dependency changes, found AI agents select
known-vulnerable dependency versions more often than humans (2.46% vs 1.64%), and that
agent-selected vulnerable versions required a major-version upgrade to reach a patched
release 36.8% of the time versus 12.9% for humans — that is, agents pick versions that
are further behind. What that study does *not* establish is the mechanism: it does not
attribute the behaviour to model training cutoffs, and does not measure version age at
all. A finding from this check is therefore a condition, not an attribution:
compatibility constraints, deliberate stability policies, and platform limits all
produce old pins for entirely legitimate reasons.

Only exact pins in direct manifests are assessed. Version ranges such as `^1.2.3`
resolve to the newest matching release, so any staleness is not the manifest's doing,
and transitive dependencies are excluded because a maintainer cannot change them
directly. A dependency is reported as stale at 365 days behind the current release; that
threshold is a documented policy choice, not an empirical constant. Lookups that fail,
and pinned versions absent from the registry, are reported separately and never scored
as staleness.
 

**Remediation steps**
- Update the dependency to a current release, or document why the older version is required.
- If the pin exists for compatibility, record that reason next to it so the constraint is visible to future maintainers and to automated review.
- Consider a version range or an automated update tool (Dependabot, Renovate) so security patches are picked up without manual intervention.
- If the pin was suggested by an AI coding assistant, check it against the current release rather than assuming the suggested version is current — a model's knowledge of a package reflects its training data, not today's registry.
